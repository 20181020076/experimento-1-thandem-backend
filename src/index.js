...
app.post("/", async (req, res) => {
  try {
    ...
    const token = jwt.sign({userName: req.body.user, sala:req.body.sala},secretKey,{expiresIn: '1h'});
    res.json({ status: "ok", redirectUrl: `${process.env.PAGE_URL}/componentes`, token });
  } catch (err) {
    res.json({ error: err });
  }
});
